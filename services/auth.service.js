...
const findByEmail = (candidateEmail) => {
  return mongo.db.collection("users").findOne({ candidateEmail });
};
...
const registration = async (input, res) => {
  ...
  const user = await findByEmail(input.candidateEmail);
  if (user) return res.status(400).send({ msg: "Email already exists" });

  // Encrypt the Password
  const salt = await bcrypt.genSalt(10);
  input.candidatePassword = await bcrypt.hash(input.candidatePassword, salt);
